...
function showGoalCelebration(){if(state.place?.place_type!=="COURT")return;const flip=(Number(localStorage.getItem("court_goal_flip")||0)+1)%2;localStorage.setItem("court_goal_flip",String(flip));const cfg=flip?{img:"/assets/new/court-ronaldo.jpg",title:"¡SIIIUUU!",text:"Tu pedido ya está en juego."}:{img:"/assets/new/court-messi.jpg",title:"¡JUGADA MAESTRA!",text:"Pedido enviado. Ahora a disfrutar el partido."};$("#goalCelebrationImage").src=cfg.img;$("#goalCelebrationTitle").textContent=cfg.title;$("#goalCelebrationText").textContent=cfg.text;const el=$("#goalCelebration");el.classList.remove("hidden");el.setAttribute("aria-hidden","false");clearTimeout(window.__goalTimer);window.__goalTimer=setTimeout(()=>{el.classList.add("hidden");el.setAttribute("aria-hidden","true")},2600)}
