...
$("#addCustomCombo").onclick=()=>{const picks=Object.entries(comboPick).filter(([,q])=>q>0);if(!picks.length)return toast("Elegí al menos un titular");picks.forEach(([id,qty])=>state.cart.push({product_id:id,qty,option_ids:[],note:"Equipo personalizado"}));Object.keys(comboPick).forEach(k=>delete comboPick[k]);$("#comboBuilderDialog").close();updateCart();toast("Tu equipo fue agregado al pedido")};
...
async function sendOrder(){if(!state.diner){$("#joinDialog").showModal();return}if(!state.cart.length)return toast("Agrega productos");const phone=($("#orderPhone")?.value||"").trim();const payload={session_id:state.session.id,diner_id:state.diner.diner_id,items:state.cart,note:phone?("Celular: "+phone):""};const btn=$("#sendOrder");btn.disabled=true;btn.textContent="Enviando…";const {data,error}=await sb.rpc("place_order_public",{p_payload:payload});btn.disabled=false;btn.textContent="Enviar pedido";if(error)return toast(error.message);state.cart=[];updateCart();$("#cartDialog").close();showGoalCelebration();toast("Pedido enviado")}
