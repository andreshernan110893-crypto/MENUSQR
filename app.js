...
async function sendOrder(){
 if(!state.diner){$("#joinDialog").showModal();return}
 if(!state.cart.length)return toast("Agrega productos");
 const phone=($("#feedbackPhone")?.value||"").trim();
 const notes=[];
 let paymentMethod="";
 if(state.place?.place_type==="COURT"){
  const stand=($("#courtStand")?.value||"").trim(),clothing=($("#courtClothing")?.value||"").trim(),payment=$("#courtPayment")?.value||"";
  if(!stand)return toast("Indica en qué grada o ubicación estás");
  if(!clothing)return toast("Agrega una referencia de cómo andas vestido");
  if(!payment)return toast("Selecciona el medio de pago");
  paymentMethod=payment;
  notes.push("Ubicación: "+stand,"Vestimenta: "+clothing,"Pago: "+payment);
  if(payment==="EFECTIVO"){
   const needs=$("#courtCashChange")?.value==="SI";
   if(needs){
    const tender=Number($("#courtCashAmount")?.value||0),total=state.cart.reduce((a,i)=>a+itemUnitTotal(i)*i.qty,0);
    if(!tender||tender<total)return toast("El monto para cambio debe ser igual o mayor al total");
    notes.push("Cambio: SÍ · paga con "+money(tender)+" · devolver "+money(tender-total));
   }else notes.push("Cambio: NO / pago exacto");
  }
 }
 if(phone)notes.push("Celular: "+phone);
 const payload={session_id:state.session.id,diner_id:state.diner.diner_id,items:state.cart,note:notes.join(" · "),payment_method:paymentMethod||null};
 const btn=$("#sendOrder");btn.disabled=true;btn.textContent="Enviando…";
 const {data,error}=await sb.rpc("place_order_public",{p_payload:payload});
 btn.disabled=false;btn.textContent="Enviar pedido";
 if(error)return toast(error.message);
 state.cart=[];updateCart();$("#cartDialog").close();showGoalCelebration();await loadMyOrders();toast("Pedido enviado");
}
...
function refreshCourtPaymentFields(){
 if(state.place?.place_type!=="COURT")return;
 const payment=$("#courtPayment")?.value||"",cash=$("#courtCashFields"),wrap=$("#courtCashAmountWrap"),preview=$("#courtChangePreview");
 cash?.classList.toggle("hidden",payment!=="EFECTIVO");
 const needs=payment==="EFECTIVO"&&$("#courtCashChange")?.value==="SI";
 wrap?.classList.toggle("hidden",!needs);
 if(preview){
  const tender=Number($("#courtCashAmount")?.value||0),total=state.cart.reduce((a,i)=>a+itemUnitTotal(i)*i.qty,0);
  preview.classList.toggle("hidden",!needs||!tender);
  if(needs&&tender)preview.textContent=tender>=total?"Cambio estimado: "+money(tender-total):"Faltan "+money(total-tender);
 }
}
["courtStand","courtClothing","courtPayment","courtCashChange","courtCashAmount"].forEach(id=>{const el=$("#"+id);if(!el)return;const key="minute90_"+id;const saved=localStorage.getItem(key);if(saved!==null)el.value=saved;el.addEventListener("input",()=>{localStorage.setItem(key,el.value);refreshCourtPaymentFields()});el.addEventListener("change",()=>{localStorage.setItem(key,el.value);refreshCourtPaymentFields()})});refreshCourtPaymentFields();
...
let __orderTrackerTimer=null,__lastTrackerStatus=null;
const trackerOrder=["RECEIVED","PREPARING","READY","DELIVERED"];
function trackerLabel(s){return {RECEIVED:"Pedido recibido",PREPARING:"Estamos preparando tu pedido",READY:"Tu pedido está listo",DELIVERED:"Pedido entregado",CANCELLED:"Pedido cancelado"}[s]||"Estado del pedido"}
function trackerStepActive(current,step){if(current==="CANCELLED")return false;return trackerOrder.indexOf(step)<=trackerOrder.indexOf(current)}
function renderOrderTracker(order){
 const box=$("#orderTracker");if(!box)return;
 if(!order){box.classList.add("hidden");return}
 box.classList.remove("hidden");box.dataset.status=order.status||"RECEIVED";
 $("#trackerTitle").textContent=trackerLabel(order.status);
 $("#trackerStatusBadge").textContent=(order.status||"RECEIVED").replace("_"," ");
 $("#trackerTotal").textContent=money(order.total);
 $("#trackerPayment").textContent=order.payment_method?("Pago: "+order.payment_method+(order.paid?" · PAGADO":"")):"Pago pendiente";
 document.querySelectorAll("[data-track-step]").forEach(el=>{const st=el.dataset.trackStep;el.classList.toggle("active",trackerStepActive(order.status,st));el.classList.toggle("current",order.status===st)});
 $("#trackerStations").innerHTML=(order.stations||[]).map(x=>'<span class="tracker-station '+safe(x.status)+'"><b>'+(x.station==="LB"?"La Bandeja":"Beer Station")+'</b> · '+safe(trackerLabel(x.status))+'</span>').join("");
 $("#trackerItems").innerHTML=(order.items||[]).map(i=>'<div><span>'+i.qty+' × '+safe(i.product_name)+'</span><small>'+safe(i.station==="LB"?"La Bandeja":"Beer Station")+'</small></div>').join("");
 if(__lastTrackerStatus&&__lastTrackerStatus!==order.status){toast(trackerLabel(order.status));if(navigator.vibrate)navigator.vibrate(order.status==="READY"?[120,80,120]:80)}
 __lastTrackerStatus=order.status;
}
async function loadMyOrders(){
 if(!state.session?.id||!state.diner?.diner_id){$("#orderTracker")?.classList.add("hidden");return}
 const {data,error}=await sb.rpc("get_my_orders_public",{p_session_id:state.session.id,p_diner_id:state.diner.diner_id});
 if(error)return;
 const list=Array.isArray(data)?data:[];
 const active=list.find(x=>!["DELIVERED","CANCELLED"].includes(x.status))||list[0];
 renderOrderTracker(active);
}
function startOrderTracking(){clearInterval(__orderTrackerTimer);loadMyOrders();__orderTrackerTimer=setInterval(loadMyOrders,3500)}
