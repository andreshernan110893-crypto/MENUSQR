...
async function init(){
 ...
 state.brands=b.data||[];state.categories=c.data||[];state.products=p.data||[];state.promos=pr.data||[];state.groups=g.data||[];state.options=o.data||[];state.links=l.data||[];
 state.place=pl.data||{code:place,name:place,place_type:"TABLE"};document.body.dataset.placeType=state.place.place_type;$("#placeName").textContent=state.place.name;
 renderHeaderLogos();state.activeCategory=state.categories[0]?.id||null;renderCategories();renderProducts();renderPromos();renderPromoGrid();restoreDiner();updateCart();setInterval(nextPromo,4800);
 if(state.diner)await refreshDiners();else setTimeout(()=>$("#joinDialog").showModal(),650);
}
function renderHeaderLogos(){const el=$("#headerLogos");el.innerHTML=state.brands.map(b=>`<img src="${b.logo_url||""}" alt="${safe(b.name)}">`).join("")}
